...
|---|---|---|---|---|---|
| `lichtspielOpus` | webgl | faderBank + arcMacros | grid64/arc2 | `Lichtspiel_v3.pde` (idiom master) | Ruttmann Opus III — **faithful WEBGL re-port** of the P3D original: a 3D morphing tunnel (filled shells + contour rings + twisting strands, lobe undulation + noise + the bulge field) + interior noisy-sphere morphs + volumetric 3D grain, inside the 2D film language (tinted backplate, Ruttmann rect forms, rect bursts, film gate + sprocket holes + 4 grain types — depth-test toggled). 8 faders + arc twist/aperture/presses. (Processing port — not a windchime TS family.) |
